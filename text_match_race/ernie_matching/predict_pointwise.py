...
def get_predict_result(resutl_out_path,text_pair_list):
    with open(resutl_out_path,'w',encoding='utf-8') as f:
        print("text_pair总条数{}".format(len(text_pair_list)))
        index_count = 0
        for text_pair in text_pair_list:
            out_resutl = str(index_count) + "\t" + str(text_pair["pred_label"])
            f.write(out_resutl)
            f.write("\n")
            index_count +=1
